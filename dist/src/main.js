...
function createMeshes() {
    ...
    plane = new Plane(vec3.fromValues(0, 0, 0), vec2.fromValues(aspectRatio * planeHeight, planeHeight), 12);
    square.create();
    screenQuad.create();
    cube.create();
    roadCube.create();
...
function initializeGenerators() {
    ...
    let translate = mat4.create(), scale = mat4.create(), planeScale = mat4.create();
    mat4.fromTranslation(translate, vec3.fromValues(aspectRatio * planeHeight / 2, controls.waterLevel + 0.15, -planeHeight / 2));
    mat4.fromScaling(scale, vec3.fromValues(-2 / Math.floor(rgCityHeight), 1, 2 / rgCityHeight));
    mat4.fromScaling(planeScale, vec3.fromValues(planeHeight / 2, 1, planeHeight / 2));
    ...
    let rgCityDimensions = vec2.fromValues(Math.floor(aspectRatio * rgCityHeight), rgCityHeight), rgGridDimensions = vec2.fromValues(Math.floor(aspectRatio * rgGridHeight), rgGridHeight);
    let cgCityDimensions = vec2.fromValues(Math.floor(aspectRatio * cgCityHeight), cgCityHeight), cgGridDimensions = vec2.fromValues(Math.floor(aspectRatio * cgGridHeight), cgGridHeight);
